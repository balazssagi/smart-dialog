...
var dialogPolyfill = createCommonjsModule(function (module) {
  (function () {
    ...
    } else if ('object' === 'object' && _typeof(module['exports']) === 'object') {
      // CommonJS support
...
function isClickInsideDialog(element, event) {
  ...
  return !(rect.top <= event.clientY && rect.left <= event.clientX && event.clientX <= rect.left + rect.width && event.clientY <= rect.top + rect.height);
}

function SmartDialog(element, options) {
  if (!isValidElement(element)) {
    throw new Error("Passed element is invalid. You should pass a 'dialog' element.");
  }
